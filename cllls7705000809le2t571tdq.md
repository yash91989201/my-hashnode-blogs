...
Hey there, enthusiastic readers! Get ready for an incredible journey as we dive into Kubernetes, a fantastic container orchestration tool that empowers us to manage multiple containers like a pro! In this blog, we'll be exploring the ins and outs of Kubernetes and also installing it! 🎉🚀

## ✔️ Why do we need container orchestration?🤔

Container orchestration is heavily required in situations where we are running our application in a microservices architecture. Here are some issues that we will face if we only use containers for serving our application:
...
The control plane has all the components that are required to manage the worker nodes. Some of the components are custom built for the control plane like controller, controller manager, scheduler, API server but etcd is a separate service that plays a very crucial role in the control plane

Here are all the components of a control plane:
...
3. **etcd:**
    ...
    * *fast:* It can perform 10,000 writes per second.
...
1. **kubelet:**
    ...
    * It is the node-level worker that makes the k8s cluster actually work.
